...
public class DB_EPREUVE
{
	...
	public DB_EPREUVE( Connection conn )
	{
		...
		try {
			...
			ps_update = conn.prepareStatement( "update epreuve set nom=?, categ=?, datep=?,tarifclub=?, tarifnonclub=? where ide=? cascade" );
			ps_delete = conn.prepareStatement( "delete cascade from epreuve where ide=? cascade" );
