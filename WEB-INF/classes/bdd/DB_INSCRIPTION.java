...
public class DB_INSCRIPTION
{
	...
	public Inscription getInscription( int idp , int ide )
	{
		...
		try {
			...
			if( rs.next( ) ) {
				int    idp1       = rs.getInt( "idp" );
				int    ide1       = rs.getInt( "ide" );
	...
	//est ignor̩e.
	public void insertInscription( Inscription p )
	{
		try {
			ps_insert.setInt( 1 , p.getIdp( ) );
			ps_insert.setInt( 2 , p.getIde( ) );
	...
	public void updateInscription( Inscription p )
	{
		try {
			...
			ps_update.setInt( 2 , p.getIdp( ) );
			ps_update.setInt( 3 , p.getIde( ) );
